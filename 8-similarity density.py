...
def pointwise_spatial_kde(
    ...
    kernel: str = "gaussian",     # 'gaussian' / 'circle' / 'combo'
    normalize: bool = True,
    bandwidth: Optional[float] = None,  # None => h = radius_m/2（仅对 gaussian/combo 用）
    leafsize: int = 40,
    ...
    num = np.zeros(n, dtype=float)   # 加权和
    den = np.zeros(n, dtype=float)   # 核权之和
...
def plot_kde_boxplots(df: pd.DataFrame, target_col: str, out_dir: str, base_name: str,
    ...
    def _boxplot_for_range(xmin, xmax, left_open, fname, title):
        ...
        # 坐标轴与标题
        ax.set_xlabel(" simsilaity OPN")   # 显示名称固定
        ...
        # —— 左上角标注 Spearman 结果 ——（置于 axes 坐标 0.02, 0.98）
        if not np.isnan(rho):
            ax.text(
                0.02, 0.98,
                f"Spearman ρ={rho:.3f}\np={pval:.3g}\nn={n}",
...
def run(csv_path: str, lon_col: str, lat_col: str,
    ...
    # 10) 出图：两张箱线图（0–50 与 50–600），并在左上角标注区间 Spearman
    try:

...
def plot_point_heatmap(
    ...
    out_file: Optional[str] = None, # renamed variable: out_file
    ...
    plt.tight_layout()
    # Updated: save the figure to out_file
    if out_file:
...
def run(csv_path: str, lon_col: str, lat_col: str,
        ...
        no_show: bool, out_pdf: Optional[str]): # renamed variable: out_pdf
    ...
    if not no_show or out_pdf: # use out_pdf here
...
def parse_args():
    ...
    # Updated: revised argument names, defaults, and help text
    p.add_argument("--out-pdf", default=r"point_density_heatmap.pdf", help="保存 PDF 路径（可为空不保存）")
    return p.parse_args()

if __name__ == "__main__":
    args = parse_args()
    # Updated: pass the out_pdf variable to run()

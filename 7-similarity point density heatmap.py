...
def plot_point_heatmap(
    ...
    out_file: Optional[str] = None, # <--- 接收输出路径
    ...
    plt.tight_layout()
    # **关键：plt.savefig(out_file) 会使用 dpi=300 的设置，并根据 out_file 扩展名保存为 PDF**
    if out_file:
...
def run(csv_path: str, lon_col: str, lat_col: str,
        ...
        no_show: bool, out_pdf: Optional[str]): # <--- 接收 PDF 输出路径

...
# ---------- Point-wise distance kernel density (excluding self) ----------
def pointwise_distance_kde(
    xy: np.ndarray,
    radius_m: float,
    kernel: str = "exponential",        # supported kernels: exponential / gaussian / linear / inverse / uniform
    scale_m: Optional[float] = None,    # Gaussian sigma / exponential lambda; None means R/3
    inv_power: float = 1.0,
    weights: Optional[np.ndarray] = None,
    normalize: bool = False,            # True = local mean; False = density (weighted sum)
    eps: float = 1.0
) -> np.ndarray:
    n = xy.shape[0]
...
def plot_point_heatmap(
    ...
    if usa_shp and os.path.exists(usa_shp):
        ...
        # Remove AK/HI/PR and other non-contiguous regions
    ...
    # Draw the figure
    # Note: dpi=300 is already set here
    ...
    if gdf_usa is not None and not gdf_usa.empty:
        ...
        # Clip the view to the boundary extent
    ...
    plt.tight_layout()
    # Note: plt.savefig(out_file) uses dpi=300 and saves to PDF based on the out_file extension
    if out_file:
...
def run(csv_path: str, lon_col: str, lat_col: str,
        ...
        no_show: bool, out_pdf: Optional[str]): # receive the PDF output path here
    ...
    # Point-wise density calculation (no raster and no boxplot here)
    ...
    # Draw the point heatmap
    if not no_show or out_pdf:
        usa_shp = r"../data/shp/cb_2018_us_state_20m.shp"
        title = f""
        plot_point_heatmap(
            df, lon=lon_col, lat=lat_col, crs_out=crs_out, value_col="kde_value",
            usa_shp=usa_shp, title=title, out_file=out_pdf, s=24, cmap="coolwarm" # pass out_pdf through
        )
...
def parse_args():
    ...
    # Updated to use a PDF output path and default value
    p.add_argument("--out-pdf", default=r"similarity_point_density_heatmap.pdf", help="保存 PDF 路径（可为空不保存）")
    return p.parse_args()

if __name__ == "__main__":
    args = parse_args()
    # Use the out_pdf variable when calling run()

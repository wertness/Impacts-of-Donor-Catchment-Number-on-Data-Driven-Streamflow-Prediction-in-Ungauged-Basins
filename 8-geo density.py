...
def station_density_pointwise(
    ...
    bandwidth: Optional[float] = None,# σ 或 λ；None则用 R/3
    inv_power: float = 1.0,
    weights: Optional[np.ndarray] = None,  # 对邻点的权重；None 等权
    normalize: bool = False,          # True=局部平均；False=密度（权重和）
    eps: float = 1.0,
) -> np.ndarray:
    n = xy.shape[0]
...
def plot_bins(df: pd.DataFrame, xcol: str, ycol: str,
    ...
    def _plot(xmin, xmax, left_open, fname, title):
        ...
        # 颜色配置
        maroon = (240/255, 65/255, 85/255)   # 酒红（箱子、均值）
        green  = "tab:green"                 # 均值线
        black  = (0, 0, 0)                   # 黑色（中位数线）
        gray   = (0.4, 0.4, 0.4)
        ...
        # 坐标轴与标题（横轴显示文本固定为 max_geo_n；标题传参）
        ax.set_xlabel(" geo OPN")   # 只改显示文字，不改数据列
        ax.set_ylabel(ycol)
        ax.set_title(title)          # ← 用传入的标题，而非写死
...
def run(csv_path: str, lon: str, lat: str,
    ...
    # ===== 选择“横轴数据列”：存在则用 max_similarity_n，否则自动挑一个 =====
    ...
    # Spearman（用 xcol 与 station_density）
